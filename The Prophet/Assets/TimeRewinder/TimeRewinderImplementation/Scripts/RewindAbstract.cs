...
public abstract class RewindAbstract : MonoBehaviour
{
    ...
    public struct HealthSystemValues
    {
        public float health;
        public short currentHealTriesCount;
    }

    CircularBuffer<HealthSystemValues> trackedHealthSystems;

    protected void TrackHealthSystem()
    {
        HealthSystemValues healthSystemValues;
        healthSystemValues.health = PlayerHealthController.instance.health;
        healthSystemValues.currentHealTriesCount = PlayerHealthController.instance.currentHealTriesCount;
        trackedHealthSystems.WriteLastValue(healthSystemValues);
    }

    protected void RestoreHealthSystem(float seconds)
    {
        HealthSystemValues valuesToRead = trackedHealthSystems.ReadFromBuffer(seconds);
        PlayerHealthController.instance.currentHealTriesCount = valuesToRead.currentHealTriesCount;
        HealPotionsBarController.instance.UpdateHealPotions();
        PlayerHealthController.instance.SetHealth(valuesToRead.health);
    }
    ...
    CircularBuffer<bool> trackedFlips;

    protected void TrackFlips()
    {
        trackedFlips.WriteLastValue(spriteRenderer.flipX);
    }

    protected void RestoreFlips(float seconds)
    {
